...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val btnSetEvent = findViewById(R.id.set_event) as Button // button for action started
        btnSetEvent.setOnClickListener{

            // define action for 'Diary' worker Action in calendar
            val intent = Intent(Intent.ACTION_INSERT)
                .setData(CONTENT_URI) // For import apply for < (in android.provider.CalendarContract) >
                .putExtra(TITLE, "Bootcamp NTT") // o < TITLE > also CalendarContract.Events.*
